...
def main(unused_argv):
    ...
    while True:
        ...
        step = int(state.step) #! change wrt newly defined TrainClass
        if step <= last_step:
            continue
        ...
        for idx in range(min(dataset.size, 10)): #! added min just for testing purposes -- use only 10 test images 
            print(f"Evaluating {idx+1}/{dataset.size}")
            batch = next(dataset)
            if idx % FLAGS.approx_eval_skip != 0:
                continue
            pred_color, pred_disp, pred_acc = utils.render_image(
                functools.partial(render_pfn, state.variables), #! change state.optimizer.target to state.variables
                batch["rays"],
                rng,
                FLAGS.dataset == "llff",
                chunk=FLAGS.chunk,
            )
            if jax.process_index() != 0:  # Only record via host 0. #! host_id renamed to process_index()
                continue
            ...
            if not FLAGS.render_path:
                 ...
                 printstats=(f"index={idx}, PSNR = {psnr:.4f}, SSIM = {ssim:.4f}") #! made it into a variable instead of just plaintly printing it
                 print(printstats) #! use variable
            ...
            if FLAGS.save_output:
                ...
                #! save stats
                with open(FLAGS.train_dir+"/summary_training_eval.txt", "a") as f:
                    f.write(f"{printstats}\n")
                #!
                
        if (not FLAGS.eval_once) and (jax.process_index() == 0): #! host_id renamed to process_index()
        ...
        #  if FLAGS.save_output and (not FLAGS.render_path) and (jax.process_index() == 0): #! host_id renamed to process_index()
