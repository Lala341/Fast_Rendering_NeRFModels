...
let guiInit = function() {
    ...
    // #! added overall reset button fxn
    $('#options-reset-btn').click(function() {
        let opt = Volrend.get_options();
        
        // view direction shifting
        sliders_vdir.val(0.0);
        opt.rot_dirs[0] = 0.0;
        opt.rot_dirs[1] = 0.0;
        opt.rot_dirs[2] = 0.0;
        ...
        //sh decompose
        opt.basis_minmax[0] = 0.0;
        opt.basis_minmax[1] = 9.0;
        document.getElementById('slider-decomp-min').value = 0.0;
        document.getElementById('slider-decomp-max').value = 9.0;
        document.getElementById('decomp-min-disp').innerText = 0.0;
        document.getElementById('decomp-max-disp').innerText = 9.0;

        //slicing
        sliders_min.val(0.0);
        sliders_max.val(1.0);
        opt.render_bbox[0] = 0.0;
        opt.render_bbox[1] = 0.0;
        opt.render_bbox[2] = 0.0;
        opt.render_bbox[3] = 1.0;
        opt.render_bbox[4] = 1.0;
        opt.render_bbox[5] = 1.0;

        //backlight
        slider_bgbrightness.val(1.0);
        opt.background_brightness = 1.0;

        //octree grid
        sliders_grid_reso.val(4);
        opt.grid_max_depth = 4.0;
        document.getElementById('grid-reso-disp').innerText = 4.0;

        Volrend.set_options(opt);
    });

    // $('#screencap-btn').click(function() {
    ...
    //         window.saveAs(blob, "plenoctree_rendered.png");
    //     });
    // });
