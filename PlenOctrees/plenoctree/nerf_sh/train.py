...
#! flax.checkpoints is depreciated. Adapting code to orbax for future-proofing
#! https://flax.readthedocs.io/en/latest/guides/training_techniques/use_checkpointing.html
flax.config.update('flax_use_orbax_checkpointing', True) 

def train_step(model, rng, state, batch, lr): #! replace arguments needed
#! def train_step(model, rng, opt_state, batch, lr, params_state): # version with no use of TrainClass
...
def main(unused_argv):
    ...
    # TEMP
    timings_file = open(os.path.join(FLAGS.train_dir, 'timings.txt'), 'a+') #! change to a + --> create if doesnt exist
    ...
    # init_step = state.optimizer.state.step + 1 #! change this
    # init_step = 1 #! start at 1 instead, assume optimizer step is 0
    init_step=state.step + 1 #! change based on the restructured TrainState class
    ...
    rng = rng + jax.process_index()  # Make random seed separate across hosts. #! host_id renamed to process_index()
    ...
    for step, batch in zip(steps, pdataset): #! change steps
        ...
        if jax.process_index() == 0: #! host_id renamed to process_index()
            if step % FLAGS.print_every == 0:
                ...
                with open(FLAGS.train_dir+"/gpu_memory_consumption_training.txt", "a") as f:
                    f.writelines(str(g)+'\n')
                #!
                ...
                #! Create intermediate oject so it's easier to store also
                s=(
                    (f"TRAIN {step} "
                ...
                print(s) #! changed 
                with open(FLAGS.train_dir+"/summary_training.txt", "a+") as f: #! a to a+
                    f.writelines(s+'\n')
                #!
            ...
            if step % FLAGS.save_every == 0:
                print('* Saving')
                state_to_save = jax.device_get(jax.tree_map(lambda x: x[0], state)) #! replaced state with opt_state -- reverted back to original
        ...
        if FLAGS.render_every > 0 and step % FLAGS.render_every == 0:
            ...
            if jax.process_index() == 0: #! host_id renamed to process_index()
                ...
                #! Create intermediate oject so it's easier to store also
                s=(f"TESTEVAL {step} "
                    + f"eval_time={eval_time:0.3f}s, "
                    + f"{rays_per_sec:0.0f} rays/sec, "
                    + f"psnr={psnr:0.4f}, "
                    + f"ssim={ssim:0.4f}"
                    )
                print(s) #! changed 
                with open(FLAGS.train_dir+"/summary_training.txt", "a+") as f: #! changed from a to a+
                    f.writelines(s+'\n')
                #!
    ...
    with open(FLAGS.train_dir+"/total_training_time_training.txt", "a+") as f: #! changed from w to a+
        f.writelines(f"Total Training Time: {total_training_end-total_training_start} seconds\n")
    #!
