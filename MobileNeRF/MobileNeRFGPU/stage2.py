...
# ! pip3 install -r "/content/drive/MyDrive/datasets_drums/requirements2.txt"
...
object_name = "drums"
scene_dir = "datasets_drums/nerf_synthetic/"+object_name
...
# synthetic
# drums drums ficus hotdog lego materials mic ship
...
if len(jax.local_devices())!=8:
  ...
  #1/0
weights_dir = "datasets_drums/nerf_synthetic/weights_100000"
samples_dir = "datasets_drums/nerf_synthetic/samples_100000"
...
optimizer = optax.inject_hyperparams(optax.adam)(learning_rate=1e-5, **adam_kwargs)
...
training_iters = 50000 #edit 400000
train_psnr_max = 0.0
step = 10000
...
for i in tqdm(range(step_init, training_iters + 1)):
  ...
  if (i % step == 0) and i > 0: # EDIT if (i % 10000 == 0) and i > 0:
    ...
    this_train_psnr = np.mean(np.array(psnrs[-int(step/2):]))

    #stop when iteration>200000 and the training psnr drops
    if i>int(training_iters/2) and this_train_psnr<=train_psnr_max-0.001:
      unreplicated_state = pickle.load(open(weights_dir+"/s2_0_"+"tmp_state"+str(i-step)+".pkl", "rb"))
...
df.to_csv(metrics_dir+'/s2_0_train_results_general_train.csv', index=False)
...
df.to_csv(metrics_dir+'/s2_0_train_results_test_train.csv', index=False)

# Save GPU memory consumption and total training time
with open(metrics_dir+"/s2_0_gpu_memory_consumption_training.txt", "w") as f:
    f.write(str(gpu_memory_consumption))

with open(metrics_dir+"/s2_0_total_training_time_training.txt", "w") as f:
...
df.to_csv(metrics_dir+'/s2_0_test_results_test.csv', index=False)
print(df)
...
df.to_csv(metrics_dir+'/s2_1_train_results_general_train.csv', index=False)

df = pd.DataFrame({
    'psnrs_b_test': psnrs_b_test,
    'iters_test': iters_test
})

df.to_csv(metrics_dir+'/s2_1_train_results_test_train.csv', index=False)
# Save GPU memory consumption and total training time
with open(metrics_dir+"/s2_1_gpu_memory_consumption_training.txt", "w") as f:
    f.write(str(gpu_memory_consumption))

with open(metrics_dir+"/s2_1_total_training_time_training.txt", "w") as f:
    f.write(f"Total Training Time: {total_training_end-total_training_start} seconds\n")
...
df.to_csv(metrics_dir+'/s2_1_test_results_test.csv', index=False)
print(df)
#%%
