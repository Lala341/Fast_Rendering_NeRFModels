...
Original file is located at
    https://colab.research.google.com/drive/1rhDFbTT8aDj-T8S5FBwp-Jib7vrNFUt-
"""
...
training_iters = 100000 #edit 200000
train_iters_cont = 150000  #edit
if scene_type=="real360":
  training_iters = 150
step_log = 10000
...
def train_step( opt_state, rng, traindata, lr, wdistortion, wbinary, wbgcolor, batch_size, keep_num, threshold, params_state):
  ...
  #current_state = flax.jax_utils.unreplicate(opt_state)
...
for i in tqdm(range(step_init, training_iters + 1)):
  ...
  wbgcolor = min(1.0, float(i)/training_iters*0.25)# edit 50000)
  wbinary = 0.0
  ...
  if i<=training_iters*0.25: #edit 50000:
    batch_size = test_batch_size//4
    keep_num = test_keep_num*4
    threshold = -100000.0
  elif i<=training_iters*0.5: #edit 100000:
  ...
  if (i % step_log == 0) and i > 0: #edit if (i % 10000 == 0) and i > 0:
    gc.collect()
...
df.to_csv(metrics_dir+'/s1_train_results_general_train.csv', index=False)
...
df.to_csv(metrics_dir+'/s1_train_results_test_train.csv', index=False)
print(df)

# Save GPU memory consumption and total training time
with open(metrics_dir+"/s1_gpu_memory_consumption_training.txt", "w") as f:
    f.write(str(gpu_memory_consumption))

with open(metrics_dir+"/s1_total_training_time_training.txt", "w") as f:
...
df.to_csv(metrics_dir+'/s1_test_results_test.csv', index=False)
